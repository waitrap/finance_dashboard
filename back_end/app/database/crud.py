...
def get_day_balance(db: Session, year: int,month: int,day: int,username:str):
    # 前回の更新日のデータを取得
    data = db.query(models.Balance).filter(models.Balance.year <= year, models.Balance.month <= month, models.Balance.day <= day,models.Balance.user == username).order_by(models.Balance.year.desc(), models.Balance.month.desc(), models.Balance.day.desc()).first()
    if data != None:
        return data.amount
    else:
        return 0